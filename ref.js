...
Adom.prototype.tokenize = function (prog, start_pos, end_pos, file) {
  ...
  tokens.push({ type: 'file_begin', data: file })
...
Adom.prototype.resolve_imports2 = function (tokens, file) {
  let out_toks = []
  let ptr = 0
  ...
  while (ptr < tokens.length) {
    switch (tokens[ptr].type) {
      case 'import': {
        ...
        let f = this.openFile(path)
        let toks = this.tokenize(f, 0, f.length - 1, path)
        toks = this.resolve_imports2(toks, path)
        out_toks.push({ type: 'file_begin', data: path })
        toks.forEach(function (t) {
          out_toks.push(t)
        })
        out_toks.push({ type: 'file_end', data: path })
      } break
      default:
        //console.log(tokens[ptr])
...
Adom.prototype.compile_file = function (file, input_state) {
  try {
    let prog = this.openFile(file)
    let tokens = this.tokenize(prog, 0, prog.length - 1, file)
    tokens = this.resolve_imports2(tokens, file)
